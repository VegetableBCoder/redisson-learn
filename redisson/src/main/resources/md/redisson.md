...
- 本次分享主要集中在映射,分布式锁两块,了解redisson的特性
...
1. 映射的顶层接口是RMap,RedissonMap是其基础的实现
2. RLocalCachedMap和RMapCache则在父接口的基础上定义了本地缓存和hash-key过期控制相关的操作
3. RedissonLocalCachedMap和RedissonMapCache都继承了RedissonMap

### RMap,常用实现类:RedissonMap

1. 无元素淘汰功能,无本地缓存功能 相当于原生的redis hash,
2. RMap通过继承ConcurrentMap接口提供了一套易于上手的API 使用的时候可以将其当做数据存储在redis上的远程对象
...
// .writeBehindBatchSize(100)
    ...
    .loader(new MapLoader<String, Object>() {
        ...
        public Iterable<String> loadAllKeys() {
            // 读取keys 可以返回一个迭代器的实现 使用时逐个获取 这里直接返回set
...
```
6. 使用场景,与redis hash一致, 需要考虑大key的问题(频繁访问的数据量本身就很大, 因为key频繁被续期而导致没有被访问的数据未被清除)
...
2. 就如同在仅有数据库的系统中引入Redis需要考虑数据一致性问题一样 额外引入一级缓存也需要考虑这个问题
3. RedissonLocalCachedMap的本地缓默认是自定义实现的,也可以使用caffeine,使用caffeine的实现时无需指定LRU/LFU淘汰算法(caffein有一结合LRU和LFU特点的W-TinyLFU淘汰算法),但依旧可以使用软引用,弱引用的方式进行清理
...
* redisson__map_cache__last_access__set:{key-name} 用zset存储的元素访问信息 LRU记录访问时间戳 LFU记录访问次数(这里有个疑问,G操作没有更新访问记录)
* redisson__timeout__set:{test-key1} key的过期时间 
* {key-name}:redisson_options map的配置信息
3. get操作覆盖了getOperationAsync方法实现了访问时间和访问次数的更新,为淘汰算法提供支持 如果数据没有idle的设置,可以考虑使用getWithTTLOnly get操作只管过期时间,不管idle时间 
4. 元素put过程需要进行key的过期检测,超过容量的情况下进行淘汰

![put流程](./MapCache/PutOperation.dio.svg)

5. 定时任务对元素进行定期淘汰 淘汰时检查
...
client2.shutdown();
 ```   
7. 适用场景:数据量不大,可以放到一个hash key内,但是数据并不都是再时间分布上均匀的被访问,而是某一段时间内频繁被访问

## 锁

### 分布式锁需要解决的问题

* redis锁利用了redis单线程模型的特性 并结合setnx操作和过期时间解决了锁的操作操作并发问题和释放失败造成死锁的问题,但是还有一些其他问题需要解决

* 如何实现锁的重入
    - 记录锁的持有者信息和重入次数 当获取锁时发现持有者是自己的时候 对重入次数+1 解锁时-1
* 如何实现避免加锁再执行的任务没有结束锁就过期的问题
    - redisson使用"看门狗机制", 对还没有释放的锁定时进行续签,锁释放后停止续签任务
...
> 分布式的并发环境下,通过对资源的锁定来避免同时进行会出现相互干扰的操作
...
* 适用场景: 并发量不大,但是要给所以获取资源者执行的机会,避免长期等不到的问题

### 读写锁

> 读写锁允许多个线程持有读锁,但只允许一个线程持有写锁,一个线程持有写锁的时候也可以再持有读锁;
...
![写锁获取](./WriteLock/Unlock.dio.svg)

* 适用场景 读多写少但是不能忽略写的时候对读的影响, 但是RedissonReadWriteLock没有实现读优先或者写优先
